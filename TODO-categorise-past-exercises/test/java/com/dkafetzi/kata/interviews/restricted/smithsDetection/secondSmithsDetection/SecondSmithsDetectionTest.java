package com.dkafetzi.kata.interviews.restricted.smithsDetection.secondSmithsDetection;
...
class SecondSmithsDetectionTest {
	...
	void testSimple(TestInfo testInfo) {
		String path_base = "hackerrank/secondSmithsDetection/testFiles/";
		...
		SecondSmithsDetection sd = new SecondSmithsDetection();
		...
		for (int i = 0; i < testCases.size(); i++) {
			try {
				...
				watch.reset();
				watch.start();
				output = String.valueOf(sd.countingKeywords(input));
				watch.stop();
